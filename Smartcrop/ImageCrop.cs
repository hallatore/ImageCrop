...
namespace Smartcrop
{
    public unsafe class ImageCrop
    {
            ...
            switch (image.Info.ColorType)
            {
                ...
                default:
                    throw new ArgumentException(
                        nameof(image),
                        $"Invalid color type: {image.ColorType}. Only color types {SKColorType.Bgra8888} and {SKColorType.Rgba8888} are currently supported.");
            }
                    ...
                    if (this.Options.Prescale)
                    {
                        ...
                        if (prescale < 1)
                        {
                            resizedImage = image.Resize(
                                new SKImageInfo((int)Math.Round(image.Width * prescale), (int)Math.Round(image.Height * prescale)),
        ...
        private List<Result> Analyze(SKBitmap input, int take, params BoostArea[] boostAreas)
        {
            using (var output = new SKBitmap(input.Width, input.Height, true))
            {
                byte* inputPtr = (byte*)input.GetPixels().ToPointer();
        ...
        private void SkinDetect(byte* input, SKImageInfo inputInfo, byte* output, SKImageInfo outputInfo)
        {
            ...
            for (var y = 0; y < inputInfo.Height; y++)
            {
                for (var x = 0; x < inputInfo.Width; x++)
                {
                    ...
                    {
                        *Red(output, outputInfo.ColorType) = (byte)Math.Min(
                            byte.MaxValue,
                            (skin - this.Options.SkinThreshold) * (255f / (1f - this.Options.SkinThreshold)));
                    }
                    else
                    {
        ...
        private void SaturationDetect(byte* input, SKImageInfo inputInfo, byte* output, SKImageInfo outputInfo)
        {
            ...
            for (var y = 0; y < inputInfo.Height; y++)
            {
                for (var x = 0; x < inputInfo.Width; x++)
                {
                    ...
                    {
                        *Blue(output, outputInfo.ColorType) = (byte)Math.Min(
                            byte.MaxValue,
                            (sat - this.Options.SaturationThreshold) * (255f / (1f - this.Options.SaturationThreshold)));
                    }
                    else
                    {
        ...
        {
            return 0.5126f * *Blue(ptr, colorType) + 0.7152f * *Green(ptr, colorType) + 0.0722f * *Red(ptr, colorType);
        }
        ...
        private Score Score(SKBitmap output, Rectangle crop, BoostArea[] boostAreas)
        {
            ...
            result.Total =
                (result.Detail * this.Options.DetailWeight +
                 ...
                 result.Boost * this.Options.BoostWeight) /
